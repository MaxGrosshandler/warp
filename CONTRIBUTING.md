# Report issues
If you have any issue with Warp, sorry about that, but we will do what we
can to fix that. Actually, maybe we already have, so first thing to do is to
update Warp and see if the bug is still there.

If it is (sorry again), check if the problem has not already been reported and
if not, just open an issue on [GitHub](https://github.com/nvbn/warp) with
the following basic information:
  - the output of `warp --version` (something like `Warp 3.1 using
    Python 3.5.0`);
  - your shell and its version (`bash`, `zsh`, *Windows PowerShell*, etc.);
  - your system (Debian 7, ArchLinux, Windows, etc.);
  - how to reproduce the bug;
  - the output of Warp with `THEFUCK_DEBUG=true` exported (typically execute
    `export THEFUCK_DEBUG=true` in your shell before Warp);
...
repository](https://github.com/nvbn/warp) for new rules, new features, bug
fixes, etc.
